...
class PostForm(forms.ModelForm): # Определение формы на основе модели Post
    ...
    class Meta:
        ...
        exclude = ('user',) # То же самое, но через исключение каких-либо полей (здесь не выводит пользователей, посты пользователь не подписывал чужими пользователями)

class PostCategoryForm(forms.ModelForm): # Определение формы на основе модели Post для Category
    name = forms.CharField( # Поле для ввода названия поста
        label='Название', # Метка для поля ввода
        widget=forms.TextInput( # Использование текстового поля для ввода
            attrs={'placeholder': 'Имя', 'class': 'form-control'})) # Атрибуты для текстового поля: плейсхолдер и класс CSS
    ...
    class Meta:
        model = Post # Инициируем модель, которая будет использоваться в форме
        exclude = ('user', 'category') # То же самое, но через исключение каких-либо полей (здесь не выводит пользователей, посты пользователь не подписывал чужими пользователями)